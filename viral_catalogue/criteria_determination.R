...
phage_table <- read_csv("VP2_all_selection1.csv")

# remove first index column
phage_table <- phage_table %>% select(-...1)
...
# remove NA rows 
phage_B4 <- phage_table_b4[!is.na(phage_table_b4$VS2),]
phage_B5 <- phage_table_b5[!is.na(phage_table_b5$VS2),]
phage_B7 <- phage_table_b7[!is.na(phage_table_b7$VS2),]
phage_B9 <- phage_table_b9[!is.na(phage_table_b9$VS2),]
phage_M <- phage_table_M[!is.na(phage_table_M$VS2),]
phage_F <- phage_table_F[!is.na(phage_table_F$VS2),]

# Determine which technology returned information
phage_B4 <- phage_B4 %>% mutate(tools = ifelse((DVF & VS2),"Both",
                            ifelse(DVF, "DVF", "VS2")))
phage_B5 <- phage_B5 %>% mutate(tools = ifelse((DVF & VS2),"Both",
                                               ifelse(DVF, "DVF", "VS2")))
phage_B7 <- phage_B7 %>% mutate(tools = ifelse((DVF & VS2),"Both",
                                               ifelse(DVF, "DVF", "VS2")))
phage_B9 <- phage_B9 %>% mutate(tools = ifelse((DVF & VS2),"Both",
                                               ifelse(DVF, "DVF", "VS2")))
phage_M <- phage_M %>% mutate(tools = ifelse((DVF & VS2),"Both",
                                               ifelse(DVF, "DVF", "VS2")))
phage_F <- phage_F %>% mutate(tools = ifelse((DVF & VS2),"Both",
                                               ifelse(DVF, "DVF", "VS2")))
...
checkV_B4 <- phage_B4 %>% count(checkv_quality) %>%  mutate(time = rep("B4",5))
checkV_B5 <- phage_B5 %>% count(checkv_quality) %>%  mutate(time = rep("B5",5))
checkV_B7 <- phage_B7 %>% count(checkv_quality) %>%  mutate(time = rep("B7",5))
checkV_B9 <- phage_B9 %>% count(checkv_quality) %>%  mutate(time = rep("B9",5))
checkV_M <- phage_M %>% count(checkv_quality) %>%  mutate(time = rep("M",5))
checkV_F <- phage_F %>% count(checkv_quality) %>%  mutate(time = rep("F",5))
